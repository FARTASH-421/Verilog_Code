...
if {[string match "*64 vsim*" [vsim -version]]} {
	...
	if {$tcl_platform(os) eq "Linux"} {
		if {$tcl_platform(machine) eq "aarch64"} {
			quietly set platform "linux_aarch64"
		} else {
...
if {$is64bit == 1 } {
	if {$tcl_platform(os) eq "SunOS"} {
		...
		if {$platform eq "linux_aarch64"} {
			quietly set CC "gcc -g -c -Wall -ansi -pedantic -fPIC -I. -I$INSTALL_HOME/include "
			quietly set LD "gcc -shared -lm $tclflags -Wl,-Bsymbolic -Wl,-export-dynamic -o "
		} else {
	...
	if {$platform eq "linux_aarch64"} {
		quietly set UCDB_LD "gcc -ldl -lm $tclflags -o "
	} else {
